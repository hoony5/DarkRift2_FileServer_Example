            ...
            using (DarkRiftReader? reader = msg.GetReader())
            {
                ...
                switch ((Tags)e.Tag)
                {
                    ...
                    case Tags.REQUSET_JOIN_PARTY_WITH_PASSWORD:
                        new PartyJoinProcessor().ProcessJoinPartyWithPasswordRequest(reader.ReadSerializable<RequestJoinPartyWithPassword>(), e);
                        break;
                    case Tags.REQUSET_EXIT_PARTY:
                        new PartyDissolveProcessor().ProcessExitPartyRequest(reader.ReadSerializable<RequestExitParty>(), e);
                        break;
                    case Tags.REQUSET_REMOVE_MEMBER:
                        new PartyDissolveProcessor().ProcessRemoveMemberRequest(reader.ReadSerializable<RequestRemoveMember>(), e);
                        break;
                    case Tags.REQUEST_DESTROY_PARTY:
                        new PartyDissolveProcessor().ProcessDestroyPartyRequest(reader.ReadSerializable<RequestDestroyParty>(), e);
                        break;
                    #endregion
